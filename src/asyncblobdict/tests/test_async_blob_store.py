...
async def test_json_set_get(backend):
    ...
    async with (
        ...
        await store_a.json.set(
            key,
            {"theme": "dark", "tags": {"ml", "ai"}, "last_login": datetime.utcnow()},
        )
        val_a = await store_a.json.get(key)
        val_b = await store_b.json.get(key)
        assert val_a == val_b
...
async def test_binary_set_get(backend):
    ...
    async with (
        ...
        await store_a.binary.set(key, data_bytes)
        loaded_bytes = await store_b.binary.get(key)
...
async def test_non_json_serializable(backend):
    ...
    async with AsyncBlobStore(
        ...
        with pytest.raises(ValueError):
            await store_a.json.set(key, lambda x: x)  # type: ignore
...
async def test_etag_conflict(backend):
    ...
    async with (
        ...
        await store_a.json.set(key, {"theme": "dark"})
        # Step 2: Force read to get real ETag
        store_a._cache.clear()
        await store_a.json.get(key)
        # Step 3: Get blob name via new API
        json_serializer = store_a.serializers["json"]
        blob_name = store_a._blob_name(key, json_serializer)
        # Step 4: Get cached ETag
        old_etag = store_a._cache[blob_name].etag
        assert old_etag is not None, "ETag should not be None for conflict test"
        # Step 5: Overwrite with store_b (disable ETag check, no cache)
        store_b.concurrency_mode = ConcurrencyMode.NONE
        await store_b.json.set(key, {"theme": "light"})
        # Step 6: Try to write with stale ETag
        with pytest.raises(ConcurrencyError):
            await store_a.core.set_bytes(
                blob_name,
...
async def test_list_keys(backend):
    ...
    async with (
        ...
        await store_a.json.set(key1, {"x": 1})
        await store_a.binary.set(key2, b"hello")
...
async def test_sync(backend):
    ...
    async with (
        ...
        await store_c.json.set(key, {"y": 42})
        with pytest.raises(BlobNotFoundError):
            await store_b.json.get(key)
        await store_c.sync()
        val_b = await store_b.json.get(key)
